            ...
            {filtered.length === 0 ? (
                ...
                <div className="space-y-3">
                    {filtered.map((tx, i) => {
                        ...
                        return (
                            <Link key={tx._id} href={`/dashboard/transactions/${tx._id}`}>
                                <motion.div
                                    initial={{ opacity: 0, y: 15 }}
                                    animate={{ opacity: 1, y: 0 }}
                                    transition={{ delay: 0.03 * i }}
                                    className="group bg-[#050505] border border-white/[0.05] hover:border-white/[0.1] rounded-xl p-4 sm:p-5 flex items-center gap-4 transition-all duration-300 cursor-pointer"
                                >
                                {/* Icon */}
