...
export default async function DashboardPage() {
    ...
    const projectStakesDocs = await ProjectStake.find({
        userId: session.user.id,
        status: 'active',
    })
        .populate("projectId", "name company slug heroBgColor")
        .sort({ investedAt: -1 })
        .lean() as any[];
    ...
    const formattedProjectStakes = projectStakesDocs.map((s: any) => {
        const pnlPositive = s.currentPnL >= 0;
        ...
        return {
            ...
            investedAmount:     `${cur}${s.investedAmount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
            currentValue:       `${cur}${s.currentValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
            currentPnL:         `${pnlPositive ? "+" : "-"}${cur}${Math.abs(s.currentPnL).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
            pnlPositive,
            yieldRange:         `${s.yieldLow}%${s.yieldHigh ? `–${s.yieldHigh}%` : "+"}`,
            status:             s.status,
        };
    });
    ...
    // Fetch active energy products
    const activeEnergyProducts = await import("@/models/ShopProduct").then(m => m.default).then(ShopProduct => ShopProduct.find({ isActive: true, category: "ENERGY" }).sort({ baseCashPrice: 1 }).lean());
    ...
    const formattedEnergyProducts = activeEnergyProducts.map((e: any) => ({
        id: e._id.toString(),
        slug: e.slug,
        name: e.name,
        tagline: e.description || "Power your home with clean energy",
        price: `From $${e.baseCashPrice?.toLocaleString() || "9,990"}`,
        energyType: e.specs?.energyType || "SOLAR",
        capacity: e.specs?.capacity || "13.5 kWh",
        img: e.heroImage || "",
    }));

    return <DashboardClient userData={userData} activePlans={formattedPlans} projectStakes={formattedProjectStakes} recentActivities={formattedActivities} shopOrders={formattedShopOrders} vehicles={formattedVehicles} energyProducts={formattedEnergyProducts} currency={user.currency || "$"} />;
}
